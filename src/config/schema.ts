 ...
 * shape so lark-cli / openclaw-style tooling can read it through the same
 * generic `ResolveSecretInput` pipeline.
 *
...
export function getMaxConcurrentRuns(cfg: AppConfig): number {
  ...
  // Reasonable upper bound — at 50+ concurrent codexs the bot box is
