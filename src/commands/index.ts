...
async function handleDoctor(args: string, ctx: CommandContext): Promise<void> {
  ...
  // OpenAI via the agent prompt, and (b) end up in any card payload
...
async function submitAccount(ctx: CommandContext): Promise<void> {
  ...
  void (async () => {
    ...
    // raw secret. lark-cli and other exec-provider consumers can read the
    // same SecretRef through the exec protocol to retrieve the
